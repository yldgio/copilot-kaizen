...
```js
await import("file:///absolute/path/to/global/copilot-kaizen/extension.mjs");
```

The `file:///` URL is required because ESM `import()` rejects bare absolute paths on Windows (`ERR_UNSUPPORTED_ESM_URL_SCHEME`). The URL is hardcoded at install time by `kaizen install` using `pathToFileURL()`. Relative imports in the real `extension.mjs` (e.g., `./lib/db.mjs`) resolve relative to the real file, not the trampoline.
