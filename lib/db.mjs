...
export function listEntries(db, { projectPath, category, limit = 50 }) {
  if (category) {
    return _all(db, `
...
export function markEntryApplied(db, id) {
  db.run(`
    UPDATE kaizen_entries
     ...
     WHERE id = ?
  `, [id])
  const row = _get(db, `SELECT id FROM kaizen_entries WHERE id = ?`, [id])
  return row ? 1 : 0
}
