...
// INVARIANT: Only writes to files that already exist (except tools/*.md which may be created).
 ...
 *  3. Update general.md auto-block
 *  4. Update/create tool-specific auto-blocks
 *  5. Mark high-frequency entries as crystallized
 *  6. Rebuild the kaizen.md index
 *
...
export function synthesize({ db, projectPath, kaizenDir, globalKaizenDir }) {
  ...
  // Step 2: Update general.md
  updateGeneralMd(kaizenDir, topEntries)
...
/**
 * Update the auto-block in general.md with the top mistake/pattern entries.
 * If general.md doesn't exist, skip silently.
 * If it has no auto-block, append one.
 *
 * @param {string} kaizenDir
 * @param {Array<{ content: string, hit_count: number, last_seen: string, category: string }>} topEntries
 */
function updateGeneralMd(kaizenDir, topEntries) {
  try {
    const generalPath = path.join(kaizenDir, 'general.md')

    // Read existing content (or skip if file doesn't exist)
    let content
    try {
      content = fs.readFileSync(generalPath, 'utf8')
    } catch {
      // general.md doesn't exist — nothing to update
      return
    }

    // Filter to mistakes for general.md (top 15)
    const mistakes = topEntries
      .filter(e => e.category === 'mistake' || e.category === 'pattern')
      .slice(0, 15)

    const newAutoContent = generateGeneralAutoContent(mistakes)

    const parsed = parseAutoBlocks(content)

    let newContent
    if (parsed.noBlock) {
      // No auto-block exists — append one at the end
      newContent =
        content.trimEnd() + '\n\n' +
        MARKER_OPEN + '\n' +
        newAutoContent + '\n' +
      ...
      // Replace existing auto-block content
      newContent = reconstructFile(parsed.prefix, newAutoContent, parsed.suffix)
    }

    fs.writeFileSync(generalPath, newContent, 'utf8')
  } catch {
    // swallow silently
  }
}
